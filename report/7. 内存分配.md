...
创建文件： **frame_allocator/mod.rs** 和 **frame_allocator/bit_allocator.rs** 。
...
**frame_allocator/bit_allocator.rs** 的实现如下：
```
...
```
> 对详细实现感兴趣的读者请自行阅读代码，这里不做过多介绍

## 内存初始化

**bit_allocator** 最重要的作用就是提供了一个可以用于管理 4K 页面的 **BitAlloc4K** ，有了 **BitAlloc4K** 我们就可以对内存进行管理了。首先我们需要对内存进行初始化，规定那一片内存是可以供我们使用的。
...
`BitAlloc4K.insert()` 划定了一片区域，声明其被占用。以后只能从这一片区域中选择页面用于分配。
...
`riscv::addr::*` 引入了 `struct Frame` 以及一些相关操作。由于 `BitAlloc4K.alloc()` 返回的是页面编号，类型为 `Option<usize>` ，所以需要将其转换为物理地址，然后通过 `Frame::of_addr` 转换为物理帧。
...
屏幕上显示的内容表示我们的内存分配与释放通过了测试。为什么？看一下 test 的实现就知道啦！
> 这里输出的均为物理地址
